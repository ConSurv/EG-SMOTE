...
class OldGeometricSMOTE(BaseOverSampler):
    ...
    >>> from gsmote import OldGeometricSMOTE # doctest: +NORMALIZE_WHITESPACE
    >>> X, y = make_classification(n_classes=2, class_sep=2,
    ... weights=[0.1, 0.9], n_informative=3, n_redundant=1, flip_y=0,
    ... n_features=20, n_clusters_per_class=1, n_samples=1000, random_state=10)
    >>> print('Original dataset shape %s' % Counter(y))
    Original dataset shape Counter({{1: 900, 0: 100}})
    >>> gsmote = OldGeometricSMOTE(random_state=1)
    ...
    def __init__(
        ...
        super(OldGeometricSMOTE, self).__init__(sampling_strategy=sampling_strategy)
